...
module TampaEventsAPI
  class V20260125GroupsGet200ResponseInnerSocialLinks < ApiModelBase
    attr_accessor :slack
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TampaEventsAPI::V20260125GroupsGet200ResponseInnerSocialLinks` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TampaEventsAPI::V20260125GroupsGet200ResponseInnerSocialLinks`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'meetup')
        self.meetup = attributes[:'meetup']
      end
    end
    ...
    def hash
      [slack, discord, linkedin, twitter, github, meetup].hash
    end
