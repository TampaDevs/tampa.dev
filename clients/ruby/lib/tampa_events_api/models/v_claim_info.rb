  ...
  class VClaimInfo < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TampaEventsAPI::VClaimInfo` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TampaEventsAPI::VClaimInfo`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @claimable.nil?
        invalid_properties.push('invalid value for "claimable", claimable cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] claimable Value to be assigned
    def claimable=(claimable)
      if claimable.nil?
        fail ArgumentError, 'claimable cannot be nil'
      end
    ...
    def hash
      [badge, group, claimable, reason].hash
    end
