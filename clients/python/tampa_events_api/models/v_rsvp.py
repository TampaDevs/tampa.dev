...
class VRsvp(BaseModel):
    ...
    status: StrictStr
    rsvp_at: StrictStr = Field(alias="rsvpAt")
    waitlist_position: Union[StrictFloat, StrictInt] = Field(alias="waitlistPosition")
    __properties: ClassVar[List[str]] = ["id", "eventId", "status", "rsvpAt", "waitlistPosition"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['confirmed', 'waitlisted', 'cancelled']):
            raise ValueError("must be one of enum values ('confirmed', 'waitlisted', 'cancelled')")
        return value
