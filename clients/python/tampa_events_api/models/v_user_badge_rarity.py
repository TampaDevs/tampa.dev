...
class VUserBadgeRarity(BaseModel):
    """
    VUserBadgeRarity
    """ # noqa: E501
    tier: StrictStr
    percentage: Union[StrictFloat, StrictInt]
    __properties: ClassVar[List[str]] = ["tier", "percentage"]

    @field_validator('tier')
    def tier_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['common', 'uncommon', 'rare', 'epic', 'legendary']):
            raise ValueError("must be one of enum values ('common', 'uncommon', 'rare', 'epic', 'legendary')")
        return value
