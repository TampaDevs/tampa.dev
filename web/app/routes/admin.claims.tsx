/**
 * Admin Group Claim Requests
 *
 * Review and approve/reject group ownership claim requests.
 */

import { Form, useFetcher } from "react-router";
import { useState } from "react";
import type { Route } from "./+types/admin.claims";
import {
  fetchClaimRequests,
  approveClaimRequest,
  rejectClaimRequest,
...
export const meta: Route.MetaFunction = () => [
  { title: "Claim Requests | Tampa.dev Admin" },
];
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    const data = await fetchClaimRequests(
    ...
    return { requests: [], pagination: null, status, error: "Failed to load claim requests" };
  }
}
...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    if (intent === "approve") {
      await approveClaimRequest(id, cookieHeader);
      return { success: true, message: "Claim request approved" };
    }
    if (intent === "reject") {
      const notes = formData.get("notes") as string | null;
      await rejectClaimRequest(id, notes || undefined, cookieHeader);
      return { success: true, message: "Claim request rejected" };
    }
...
function ClaimRequestRow({ request: req }: { request: ClaimRequest }) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="space-y-1 text-sm text-gray-600 dark:text-gray-400">
            ...
            {req.groupUrlname && (
              <p>
                <span className="font-medium">Group slug:</span> {req.groupUrlname}
              </p>
            )}
            {req.groupPlatform && (
              <p>
                <span className="font-medium">Platform:</span>{" "}
                <span className="capitalize">{req.groupPlatform}</span>
              </p>
            )}
            {req.notes && (
              <p>
...
export default function AdminClaimRequests({ loaderData, actionData }: Route.ComponentProps) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Review and manage group ownership claims
          </p>
        </div>
      </div>
      ...
      {actionData?.success && (
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-4">
          <p className="text-green-700 dark:text-green-400">{actionData.message}</p>
        </div>
      )}
      ...
      {requests.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="mt-1 text-gray-500 dark:text-gray-400">
            {status ? `No ${status} claim requests found.` : "No group claim requests have been submitted yet."}
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {requests.map((req) => (
            <ClaimRequestRow key={req.id} request={req} />
          ))}
        </div>
      )}
    </div>
