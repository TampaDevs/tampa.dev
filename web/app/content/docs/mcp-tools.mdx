...
The Tampa Devs MCP server exposes tools organized by domain. Each tool maps to one or more REST API operations and is gated by OAuth scopes. Use `tools/list` to discover available tools at runtime, or refer to this page for the complete reference.
...
```json
{
  ...
  "result": {
    "content": [
      {
        "type": "text",
        "text": "{\"data\":{\"success\":true,\"badge\":{\"id\":\"bdg_abc123\",\"name\":\"Early Adopter\",\"description\":\"One of the first to join\"}}}"
      }
    ]
  }
}
