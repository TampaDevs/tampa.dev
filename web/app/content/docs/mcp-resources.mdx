...
MCP resources provide read-only access to Tampa Devs platform data through a `tampadev://` URI scheme. Resources are designed for AI agents that need to load context about the platform -- events, groups, profiles, badges, and more -- without invoking tools.
