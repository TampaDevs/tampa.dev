...
export function eventToIcal(event) {
    ...
    return {
        start: moment.tz(event.data.dateTime, "America/New_York").utc().format('YYYY-M-D-H-m').split("-").map(v => parseInt(v, 10)),
        ...
        lastModified: moment.tz(new Date(), "America/New_York").utc().format('YYYY-M-D-H-m').split("-").map(v => parseInt(v, 10)),
