...
export function registerAll() {
  ...
  registerAsyncHelper(Handlebars, 'add_utm', function (options, context) {
    return new Promise((resolve, reject) => {
      resolve(
        options +
          '?utm_source=td_events_api_widget&utm_medium=organic&utm_campaign=td_events_api_widget_embed'
      );
    });
  });
  ...
  registerAsyncHelper(Handlebars, 'to_local_date', function (options, context) {
    return new Promise((resolve, reject) => {
      ...
      const dateStr = options instanceof Date ? options.toISOString() : options;
      resolve(util.toLocalDate(dateStr));
    });
  });
