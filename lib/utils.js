...
export async function filterEventData(event_data, params) {
    // Filter events that have passed their date + complete duration + 1 hour
    event_data = Object.keys(event_data).reduce((a, prop) => {
        if (event_data[prop]["unifiedEvents"]["count"] == 0)
            return a;

        a[prop] = structuredClone(event_data[prop]);
        a[prop]["unifiedEvents"]["edges"] = [];

        for (let i = 0; i < event_data[prop]["unifiedEvents"]["edges"].length; i++) {
            let event_datetime = event_data[prop]["unifiedEvents"]["edges"][i]["node"]["dateTime"]
            let event_duration = event_data[prop]["unifiedEvents"]["edges"][i]["node"]["duration"]
            if (event_datetime && moment.tz(event_datetime, "America/New_York").utc().add(moment.duration(event_duration)).add(1, 'hours') > moment.tz(new Date(), "America/New_York").utc()) {
                a[prop]["unifiedEvents"]["edges"].push(event_data[prop]["unifiedEvents"]["edges"][i]);
            }
        }
    ...
    if (params.noonline) {
        event_data = Object.keys(event_data).reduce((a, prop) => {
            ...
            for (let i = 0; i < event_data[prop]["unifiedEvents"]["edges"].length; i++) {
                if (event_data[prop]["unifiedEvents"]["edges"][i]["node"]["venue"] && event_data[prop]["unifiedEvents"]["edges"][i]["node"]["venue"]["name"] !== "Online event") {
                    a[prop]["unifiedEvents"]["edges"].push(event_data[prop]["unifiedEvents"]["edges"][i]);
                }
            }
    ...
    if (params.within_hours) {
        ...
        for (const [_, value] of Object.entries(event_data)) {
            const upcomingEvents = value.unifiedEvents.edges.filter(edge => {
            ...
            value.unifiedEvents.edges = upcomingEvents;
            value.unifiedEvents.count = upcomingEvents.length;
        }
    }
...
export function getSortedGroupNames(data) {
    const groupsArray = Object.keys(data)
        .map(key => {
            const events = data[key].unifiedEvents.edges;
