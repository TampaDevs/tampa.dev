...
export async function filterEventData(event_data, params) {
    // Filter events with dates that have already passed 
    event_data = Object.keys(event_data).reduce((a, prop) => {
        ...
        for (let i = 0; i < event_data[prop]["eventSearch"]["edges"].length; i++) {
            if (event_data[prop]["eventSearch"]["edges"][i]["node"]["dateTime"] && moment.tz(event_data[prop]["eventSearch"]["edges"][i]["node"]["dateTime"], "America/New_York").utc() > moment.tz(new Date(), "America/New_York").utc()) {
