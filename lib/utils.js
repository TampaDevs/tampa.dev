export async function filterEventData(event_data, params) {
    ...
    if (params.noonline) {
        event_data = Object.keys(event_data).reduce((a, prop) => {
            if (event_data[prop]["eventSearch"]["count"] > 0 && event_data[prop]["eventSearch"]["edges"]["0"]["node"]["venue"] && event_data[prop]["eventSearch"]["edges"]["0"]["node"]["venue"]["name"] !== "Online event") {
                a[prop] = event_data[prop];
            }
            return a;
        }, {});
