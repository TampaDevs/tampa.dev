...
export async function filterEventData(event_data, params) {
    // Filter events that have passed their date + complete duration + 2 hours
    event_data = Object.keys(event_data).reduce((a, prop) => {
        ...
        for (let i = 0; i < event_data[prop]["unifiedEvents"]["edges"].length; i++) {
            ...
            if (event_datetime && moment.tz(event_datetime, "America/New_York").utc().add(moment.duration(event_duration)).add(2, 'hours') > moment.tz(new Date(), "America/New_York").utc()) {
    ...
    if (params.noonline) {
        event_data = Object.keys(event_data).reduce((a, prop) => {
            ...
            for (let i = 0; i < event_data[prop]["unifiedEvents"]["edges"].length; i++) {
                let venue = event_data[prop]["unifiedEvents"]["edges"][i]["node"]["venue"];

                if (venue == null || (venue && venue["name"] !== "Online event")) {
                    a[prop]["unifiedEvents"]["edges"].push(event_data[prop]["unifiedEvents"]["edges"][i]);
                }
            }
