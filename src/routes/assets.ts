 ...
 * Static Assets Route
 *
 * Serves brand assets for developer integrations.
...
function createAssetsRoutes() {
  const app = new Hono<{ Bindings: Env }>();

  /**
   * GET /assets/signin-button.svg - "Sign in with Tampa.dev" button
   *
   * Query params:
   *   theme: 'light' (default) | 'dark'
   *
   * Returns an SVG image suitable for use in <img> tags.
   */
  app.get('/signin-button.svg', (c) => {
    const theme = c.req.query('theme') === 'dark' ? 'dark' : 'light';

    const bg = theme === 'light' ? '#FFFFFF' : '#1A1A1A';
    const border = theme === 'light' ? '#DADCE0' : '#3A3A3A';
